...
class CultureInfo
{
	...
	protected $_arMonths;
	protected $_arWeekdays;

	public function __construct($sLang)
	{
		if ($sLang->matches('/[a-z]{2}.{1}[A-Z]{2})/'))
		{
			include(DIANA_CORE . 'Util/CultureInfo/' . $sLang->substring(0, 2) . '-' . $sLang->substring(3) . '.php');
			$this->_arDateFormat = $arDateFormat;
			$this->_arMonths = $arMonths;
			$this->_arWeekdays = $arWeekdays;
		}
		else
		{
			throw new \Exception("No such Language: $sLang");
		}

	}

	public function translateMonth($sMonth)
	{
		return array_key_exists($sMonth->toLower(), $this->_arMonths) ? $this->_arMonths[$sMonth->toLower()] : null;
	}

	public function translateWeekday($sDay)
	{
		return array_key_exists($sDay->toLower(), $this->_arWeekdays) ? $this->_arWeekdays[$sDay->toLower()] : null;
	}
}
?>
