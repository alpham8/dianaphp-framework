...
namespace Diana\Core\Persistence\Sql
{
	...
	class BaseModel
	{
		...
		protected function _findTableFKComplex($sTableName)
		{
			...
			{
				$sTableModel = "App\\Mvc\Model\\" . $this->_decodeTableName($sJoin)->__toString() . 'Model';
				$mdl = new $sTableModel();
				$arKeys = $mdl->getKeys();
		...
		public function buildSelect(array $arCriteria = null)
		{
			...
			if (array_filled($arCriteria) && array_key_exists('distinctField', $arCriteria))
			{
			...
			if (array_filled($arCriteria) && array_key_exists('aggregate', $arCriteria))
			{
			...
			if (array_filled($arCriteria) && array_key_exists('columns', $arCriteria))
			{
				foreach ($arCriteria['columns'] as $sColumn)
				{
					$strColumn = $sColumn instanceof String ? $sColumn : new String($sColumn . '');
					$this->_sSelect = new String($this->_sSelect . ' ' . $strColumn . ' AS ' . SQL_ESC . $strColumn->replace(SQL_ESC, '') . SQL_ESC . ',');
				}
			}
			...
			else
			{
				...
				foreach ($this->_arJoin as $sJoin)
				{
					if (is_array($sJoin))
					{
						...
						$sJoinModel = "App\\Mvc\\Model\\" . $this->_decodeTableName($sJoinName) . 'Model';
						$obj = new $sJoinModel();
					...
					{
						$sJoinModel = "App\\Mvc\\Model\\" . $this->_decodeTableName($sJoin) . 'Model';
						$obj = new $sJoinModel();
						$arColumns = $obj->getColumns();
			...
			foreach ($this->_arWhere as $arActWhere)
			{
				...
				switch($arCriteria[self::CRITERIA_OPERATOR])
				{
					...
					case self::CRITERIA_IS_NULL:
						$this->_sWhere = new String($this->_sWhere . $sKey . ' IS NULL');
						break;
				}
			}
		...
		{
			$sMethod = 'set' . $this->_decodeTableName($sName);
			$sType = $sType->toLower();
				...
				if (count($arTableColumnPair) === 2)
				{
					...
					if (!$arTableColumnPair[0]->equals($this->_sTableName))
					{
						...
						foreach ($this->_arJoin as $sJoin)
						{
							...
							{
								$sModel = "App\\Mvc\\Model\\" . $this->_decodeTableName($sTable)->__toString() . 'Model';
		...
		public function fetch(array $arCriteria = null)
		{
			...
			elseif (array_filled($this->_arColumnData))
			{
			...
			foreach ($arRow as $sKey => $sValue)
			{
				if ($sValue !== null)
				{
					...
					elseif ($sTestTable->matches('/table[0-9]+/'))
					{
						...
						foreach ($this->_arJoin as $arTable)
						{
							if (is_array($arTable))
							{
								...
								{
									$sMdl = "App\\Mvc\\Model\\" . $this->_decodeTableName($sFTable)->__toString() . 'Model';
									$mdl = new $sMdl();
					...
					else
					{
						...
						if (!array_key_exists($sKeyName, $this->_arObjects))
						{
							$sModelName = "App\\Mvc\\Model\\" . $this->_decodeTableName($sKeyName) . 'Model';
							$obj = new $sModelName();
							$this->_arObjects[$sKeyName] = $obj;
							$arForeignColumns[$sKeyName] = $obj->getColumns();
						}

						$this->_arObjects[$sKeyName]->parseColumn($sColumnKey, $sValue, array_key_exists($sColumnKey->__toString(), $arForeignColumns[$sKeyName]) ? new String($arForeignColumns[$sKeyName][$sColumnKey->__toString()]) : new String('int'));
					}
		...
		public function fetchAll(array $arCriteria = null)
		{
			...
			$sModel = "App\\Mvc\\Model\\" . $this->_decodeTableName($this->_sTableName)->__toString() . 'Model';
			$arObjs = array();
			while ($result)
			{
				$mdlObj = new $sModel();
		...
		public function __call($sName, $argv)
		{
			...
			elseif($sName->toLower()->startsWith('get'))
			{
				...
				else
				{
					...
					{
						return array_key_exists($sColumn, $this->_arColumnData) ? $this->_arColumnData[$sColumn] : null;
					}
